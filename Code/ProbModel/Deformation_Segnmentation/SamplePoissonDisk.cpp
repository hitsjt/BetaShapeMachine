...
SamplePoissonDisk::SamplePoissonDisk(vector<ivec3> face_list, vector<vec3> position, vector<vec3> normal)
{
	...
	{
		Vector3 v[3] = { Vector3(position[face_list[i][0]][0], position[face_list[i][0]][1], position[face_list[i][0]][2]),
			Vector3(position[face_list[i][1]][0], position[face_list[i][1]][1], position[face_list[i][1]][2]),
			Vector3(position[face_list[i][2]][0], position[face_list[i][2]][1], position[face_list[i][2]][2])};
...
{
	return true;
	const int numRays = 32; // UNDONE: param
	float eps = (float)(mSampleRadius * 0.01);

	bool hitFlag = true;
#pragma omp parallel for shared(hitFlag)
	for (int rayID = 0; rayID<numRays; rayID++) {

		if (!hitFlag) continue;

		double r1 = getPreciseRandomNumber();
		double r2 = getPreciseRandomNumber();
		vec3 rayDir = vec3(
			2.0 * cos(M_PIf*r1*2) * sqrt(r2*(1 - r2)),
			2.0 * sin(M_PIf*r1 * 2) * sqrt(r2*(1 - r2)),
			1.0 - 2.0*r2); // random direction on unit sphere
		if ((rayDir DOT sample.normal) < 0) {
			rayDir = -rayDir; // random direction on the hemisphere above sample's tangent plane
		}
		...
		float dist = (float)tree->rayIntersectionTime(ray);
