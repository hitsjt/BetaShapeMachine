#include "Compute_Feature.h"
 ...
 int clamp(int value , int lower , int uper)
 {
	 if(value < lower)
	 {
		 return lower;
...
void Compute_Feature::exportPCA()
{
	...
	for (size_t i = 0; i < sample_set.size(); i++)
	{
		vec3 pos = sample_set[i].position;
		if(b_max[0] < pos[0])
		{
			b_max[0] = pos[0];
		}
	...
	for (size_t i = 0; i < sample_set.size(); i++)
	{
		...
		sample_set[i].pca[2] = abs(e_v(0)/ sum_e);*/
		//sample_set[i].sample_features.clear();
		if(sample_set[i].sample_features.size() > 4)
		{
			sample_set[i].sample_features[sample_set[i].sample_features.size() - 1] = (sample_set[i].pca[2]);
			sample_set[i].sample_features[sample_set[i].sample_features.size() - 2] = (sample_set[i].pca[1]);
			sample_set[i].sample_features[sample_set[i].sample_features.size() - 3] = (sample_set[i].pca[0]);
		}
		else
		{
...
void Compute_Feature::exportShapeContexNew()
{
	int NUM_RADIUS_BINS = 5;
	int NUM_ANGLE_BINS = 4;
	int NUM_BINS =  NUM_RADIUS_BINS * NUM_ANGLE_BINS;
	for(size_t i = 0; i < sample_set.size(); i++)
	{
		sample_set[i].shape_context.resize(NUM_BINS,0);
	}
	vector<double> radiusBins;
	if (radiusBins.empty()) {
	...
	float shapeScale = dist(bbMax,bbMin);
	cout << "shapeScale" << shapeScale << endl;
	for (int sampleID = 0; sampleID < sample_set.size(); sampleID++) 
	{
		vec3 sampleP = sample_set[sampleID].position;
		vec3 sampleN = sample_set[sampleID].normal;
		vector<float> hist(NUM_BINS, 0);
		for (int pointID = 0; pointID < sample_set.size(); pointID++) 
		{
			vec3 pointP = sample_set[pointID].position;
			double radius = dist(pointP ,sampleP) / shapeScale; // [0, 1]
			double angle = acos((pointP-sampleP) DOT (sampleN)); // [0, pi]
			int binR = 0;
			for (; binR < NUM_RADIUS_BINS; binR++) if (radius <= radiusBins[binR]) break;
			int binA = (int)(angle / M_PI * NUM_ANGLE_BINS);
			binR = clamp(binR, 0, NUM_RADIUS_BINS - 1);
			binA = clamp(binA, 0, NUM_ANGLE_BINS - 1);
			int binPos = binA * NUM_RADIUS_BINS + binR;
			double weight = 1 / (radius + 0.1);
			hist[binPos] += weight;
		}

		auto &feature = sample_set[sampleID].shape_context;
		...
		for (int binID = 0; binID < NUM_BINS; binID++) 
		{
			...
			sample_set[sampleID].sample_features.push_back(feature[binID]);
		}
		...
		for( size_t j = 0; j < NUM_BINNING_TYPES_GD; j++ ) 
		{
			for( size_t x = 0; x < NUM_GD_BINS[j]; x++ ) 
			{
				for( size_t y = 0; y < NUM_ANGLE_BINS[j]; y++ ) 
				{
				    ...
				    sample_set[i].sample_features.push_back(bins[j][x][y].value);
					ii++;
				}
			}
		}
...
void Compute_Feature::exportDifferent()
{
	for (size_t i = 0; i < sample_set.size(); i++)
	{
		sample_set[i].diff_feature.clear();
		vec3 diff_pos(0.0f,0.0f,0.0f);
		vector<float> diff_sdf;
		diff_sdf.resize(sample_set[i].sdf.size(),0);
		vector<float> diff_sc;
		diff_sc.resize(sample_set[i].shape_context.size(),0);
		float diff_agd = 0;
		
		for(size_t j = 0; j < near_points[i].size(); j++)
		{
			int near_id =  near_points[i][j];
			diff_pos += sample_set[i].position - sample_set[near_id].position*(1.0f/near_points[i].size());
			diff_agd += sample_set[i].angel_geo_dis[0] - sample_set[near_id].angel_geo_dis[0]*(1.0f/near_points[i].size());
			for(size_t jj = 0; jj < diff_sdf.size(); jj++)
			{
				diff_sdf[jj] += sample_set[i].sdf[jj] - sample_set[near_id].sdf[jj]*(1.0f/near_points[i].size());
			}
			for(size_t jj = 0; jj < diff_sc.size(); jj++)
			{
				diff_sc[jj] += sample_set[i].shape_context[jj] - sample_set[near_id].shape_context[jj]*(1.0f/near_points[i].size());
			}
		}
		//cout << diff_agd << endl;
		//cout << diff_pos << endl;
		sample_set[i].diff_feature.push_back(diff_pos[0]/near_points[i].size());
		sample_set[i].diff_feature.push_back(diff_pos[1]/near_points[i].size());
		sample_set[i].diff_feature.push_back(diff_pos[2]/near_points[i].size());
		sample_set[i].diff_feature.push_back(diff_agd/near_points[i].size());
		for(size_t jj = 0; jj < diff_sdf.size(); jj++)
		{
			sample_set[i].diff_feature.push_back(diff_sdf[jj]/diff_sdf.size()/near_points[i].size());
		}
		for(size_t jj = 0; jj < diff_sc.size(); jj++)
		{
			sample_set[i].diff_feature.push_back(diff_sc[jj]/diff_sdf.size()/near_points[i].size());
		}
	}
