...
void Shape::OutPutSampleAndFeature(string seg_file, string s_file, string f_file)
{
	ifstream fin(seg_file);
	vector<int> face_seg_id;
	face_seg_id.clear();
	int seg_id;
	while(fin >> seg_id)
	{
		face_seg_id.push_back(seg_id);
	}
	ofstream fout(s_file);
	ofstream foutf(f_file);
	for(int part = 0; part < 12; part++)
	{
		for(size_t i = 0; i < shape_sample.size(); i++)
		{
			if(face_seg_id[shape_sample[i].face_id] == part)
			{
				...
				fout << face_seg_id[shape_sample[i].face_id] << endl;
				for(size_t j = 0; j < shape_sample[i].sdf.size(); j++ )
				{
					foutf << shape_sample[i].sdf[j] <<" ";
				}
				for(size_t j = 0; j < shape_sample[i].shape_context.size(); j++ )
				{
					foutf << shape_sample[i].shape_context[j] <<" ";
				}
				foutf << endl;
			}
...
void Shape::GetShapeFeature()
{
	...
	test.exportSDF(Shape_Mesh);
	test.exportShapeContex();
	shape_sample = test.GetFeaturedSample(0); 
}

void Shape::WriteShapeFeature(string file_name)
{
	ofstream fout(file_name) ;
	fout << shape_sample.size() << endl;
	fout << shape_sample[0].sample_features.size() << endl;
	for(size_t i=0;i< shape_sample.size();i++)
	{
		fout <<(shape_sample[i].position[0] )<<" "<< (shape_sample[i].position[1] )<<" "<< (shape_sample[i].position[2] )<<" ";
		fout << shape_sample[i].normal[0] <<" "<< shape_sample[i].normal[1] <<" "<< shape_sample[i].normal[2] <<" " << endl;
		for(size_t j = 0; j < shape_sample[i].sample_features.size(); j++ )
		{
			fout << shape_sample[i].sample_features[j] <<" ";
		}
		fout << endl;
	}
}
...
void Shape::WriteShapeWithName(string file_name)
{
	...
	for(size_t i=0;i< shape_sample.size();i++)
	{
		...
		fout<<  (int)(shape_sample[i].pca[0]*255) <<" "<< (int)(shape_sample[i].pca[1]*255)  <<" "<< (int)(shape_sample[i].pca[2]*255) <<endl;
	}
}
...
void Shape::SaveSampleSDF(string file_name)
{
	ofstream fout(file_name);
	fout << "ply" << endl;
	fout << "format ascii 1.0" << endl;
	...
	for (size_t j = 0; j < shape_sample.size(); j++)
	{
			...
			fout << shape_sample[j].normal[0] << " " << shape_sample[j].normal[1] << " " << shape_sample[j].normal[2] << " ";
			fout << 255 << " " << 0  << " " << 0  << endl;
			//fout << (int)(shape_sample[j].sdf[0]*255) << " " << (int)(shape_sample[j].sdf[1]*255) << " " << (int)(shape_sample[j].sdf[2]*255)<< endl;
			//fout << (int)(shape_sample[j].angel_geo_dis[0]/3.0*255) << " " << (int)(shape_sample[j].angel_geo_dis[0]/3.0*255) << " " << (int)(shape_sample[j].angel_geo_dis[0]/3.0*255)<< endl;
			//float volor = 0.0;
			//for(size_t i = 0; i < shape_sample[j].diff_feature.size(); i++)
			//{
			//	volor+= abs(shape_sample[j].diff_feature[i]);
			//}
			////fout << (int)(shape_sample[j].angel_geo_dis[0]/3.0*255) << " " << (int)(shape_sample[j].sdf[0]*255) << " " << (int)(shape_sample[j].sdf[1]*255)  << endl;
			////fout << (int)(shape_sample[j].diff_feature[0] *255) << " " << (int)(shape_sample[j].diff_feature[3] *255) << " " << (int)(shape_sample[4].diff_feature[2] *255)  << endl;
			//fout << (int)abs(shape_sample[j].curvature[0]*30) << " " << (int)abs(shape_sample[j].curvature[6]*30) << " " << (int)abs(shape_sample[j].curvature[7]*30)  << endl;
	}
