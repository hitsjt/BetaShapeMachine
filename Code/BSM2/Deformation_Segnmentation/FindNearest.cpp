...
vector<vector<int>> FindNearest::FindKnearestByFeature()
{
	...
	int dim = 3 + s_sample[0].sample_features.size();  // pos + sdf + average
	...
	for (size_t i = 0; i< s_sample.size(); i++)
	{
		...
		for(size_t j =0; j < s_sample[i].sample_features.size(); j++)
		{
			dataPts[i][3 +j] = s_sample[i].sample_features[j] * 0.1;
		}
	}
	...
	/*	for(size_t j =0; j < t_sample[i].sdf.size(); j++)
		...
		for(size_t j =0; j < t_sample[i].sample_features.size(); j++)
		{
			queryPt[3 +j] = t_sample[i].sample_features[j]* 0.1;
		}
